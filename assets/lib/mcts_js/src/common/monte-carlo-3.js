...
const MonteCarloNode = require('./monte-carlo-node-3.js');
...
class MonteCarlo {
  ...
  runSearch(state, timeout = 3) {
    ...
    while (Date.now() < end && !rootNode.subtree_completed) {
  ...
  select(state) {
    ...
    while(node.isFullyExpanded() && !node.isLeaf()) {
      ...
      for (let play of plays) {
        ...
        if (!node.childTreeCompleted(play)) {
  ...
  backpropagate(node, winner) {
    let sTreeCompletion=false;
    
    if (node.isLeaf()) {
      sTreeCompletion=true;
    }
    ...
    while (node !== null) {
      ...
      if (sTreeCompletion) {
        if (node.subtree_completed===false && node.number_of_subTreeCompleted===node.number_of_children) {
          node.subtree_completed=true;
        }
        if (node.parent!==null) {
          let myPlay=node.state.playHistory[node.state.playHistory.length-1];
          let entry=node.parent.children.get(myPlay.hash());
          entry.subTreeCompleted=true;
          node.parent.children.set(myPlay.hash(),entry);
          node.parent.number_of_subTreeCompleted++;
        }
      }
