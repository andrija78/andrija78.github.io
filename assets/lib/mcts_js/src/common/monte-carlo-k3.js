...
class MonteCarlo {
  ...
  backpropagate(node, winner) {
    //let lWinner=winner.winner-1;
    //if (lWinner===-1) lWinner=2;

    while (node !== null) {
      node.n_plays++;
      // Parent's choice
      //if (node.state.isPlayerK3(-winner.winner)) {
      //if (node.state.isPlayerK3(lWinner)) {
      if (node.parent!==null && node.parent.state.player===winner.winner) node.n_wins++;
      node = node.parent;
    }
  }
  ...
  getStats(state) {
    let node = this.nodes.get(state.hash());
    let stats = { n_plays: node.n_plays, n_wins: node.n_wins, n_ratio: node.n_plays>0?node.n_wins/node.n_plays:null, children: [] };
    for (let child of node.children.values()) {
      if (child.node === null) stats.children.push({ play: child.play, n_plays: null, n_wins: null});
      else stats.children.push({ play: child.play, n_plays: child.node.n_plays, n_wins: child.node.n_wins, n_ratio: child.node.n_plays>0?child.node.n_wins/child.node.n_plays:null});
    }
    return stats;
  }
  ...
  getTreeStats(state, stats=null) {
    ...
    let nodeStats = { 
      ...
      n_wins: node.n_wins, 
      n_ratio: node.n_plays>0?node.n_wins/node.n_plays:null, 
