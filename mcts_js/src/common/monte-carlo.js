...
import MonteCarloNode from './monte-carlo-node.js';
...
class MonteCarlo {
  ...
  backpropagate(node, winner) {
    ...
    while (node !== null) {
      node.n_plays += 1;
      // Parent's choice
      //if (node.state.isPlayer(-winner.winner)) node.n_wins++;
      if (node.parent!==null && node.parent.state.isPlayer(winner.winner)) node.n_wins++;
