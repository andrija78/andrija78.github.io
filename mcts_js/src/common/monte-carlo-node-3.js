...
class MonteCarloNode {
  ...
  constructor(parent, play, state, unexpandedPlays) {
    ...
    this._isFullyExpanded=false;

    // Tree stuff
    this.parent = parent;
    this.children = new Map();
    for (let play of unexpandedPlays) {
      this.children.set(play.hash(), { play: play, node: null, subTreeCompleted: false });
    }

    this.number_of_children=unexpandedPlays.length;
    this.number_of_subTreeCompleted=0;
    
    this.subtree_completed = this.number_of_children===0;

    this._allPlays=unexpandedPlays;

    /*
    if (this.isLeaf()) {
      console.log('Leaf node', state.playHistory.map(e=>e.card).join('-'));
    }
    */
  }
  ...
  expand(play, childState, unexpandedPlays) {
    ...
    this.children.set(play.hash(), { play: play, node: childNode, subTreeCompleted: false});

    this._isFullyExpanded=true;
  ...
  getUCB1(biasParam) {
    ...
    return (this.n_belle_won / (this.n_belle_won+this.n_belle_lost)) + Math.sqrt(biasParam * Math.log(this.parent.n_plays) / this.n_plays);
  }
